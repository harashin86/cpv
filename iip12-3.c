...
void processing(void){
  int i,j,x,y,gaso=0;
  double k=0.0,filter_coefficient[3][3]={{0.0,-1.0,0.0},{-1.0,5.0,-1.0},{0.0,-1.0,0.0}};
  printf("＜フィルタ係数＞\n");
  for(i=0;i<3;i++){
    for(j=0;j<3;j++){
      printf(" %f",filter_coefficient[j][i]);
    }
    printf("\n");
  }
  for(i=0;i<height;i++){
    for(j=0;j<width;j++){
      ...
      imgout[1][j][i]=128;
      imgout[2][j][i]=128;  
      if(i==0||j==0||i==height-1||j==width-1){//枠を黒に
	imgout[0][j][i]=16;
      }else{
	for(x=0;x<3;x++){
	  for(y=0;y<3;y++){
	    k+=filter_coefficient[x][y] * imgin[0][j-1+x][i-1+y];
	  }
	}
	gaso=(int)(k+0.5);//四捨五入
	if(gaso<0){
